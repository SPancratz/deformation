                ...
                const mat_t M, const ctx_t ctxM)
{
    ...
    /* Solve the differential system iteratively */
    {
        ...
        for (i = 0; i < N - 1; i++)
        {
            _padic_mat_zero(C + (i + 1));

            j = FLINT_MAX(0, i - lenB + 1);
            for ( ; j <= i; j++)
            {
                /* C[i+1] = C[i+1] + b[i-j] * C[j]; */
                _padic_mat_mul(mat, B + (i - j), C + j, pctx);
                _padic_mat_reduce(mat, pctx);
                _padic_mat_add(C + (i + 1), C + (i + 1), mat, pctx);
            }
            ...
            for ( ; j <= i; j++)
            {
                ...
                _padic_mat_scalar_mul_fmpz(mat, C + j, coeff, pctx);
                _padic_mat_add(C + (i + 1), C + (i + 1), mat, pctx);
            }
        ...
        _padic_mat_clear(mat);
        fmpz_clear(coeff);
    }

    /* Clean-up */
    for (i = 0; i < lenB; i++)
        _padic_mat_clear(B + i);
    free(B);
